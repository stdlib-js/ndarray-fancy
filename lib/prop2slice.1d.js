...
function prop2slice( target, property, strict ) {
	...
	if ( ch === 'S' ) {
		...
		if ( s === null ) {
			throw new Error( format( '1j7En', property ) );
		}
	...
	// Case: subsequence string
		...
		if ( s.code ) {
			if ( s.code === 'ERR_SLICE_INVALID_INCREMENT' ) {
				throw new Error( format( '1j7Eq', property ) );
			}
			if ( s.code === 'ERR_SLICE_INVALID_ELLIPSIS' ) {
				throw new Error( format( '1j7Er', property ) );
			}
			if ( s.code === 'ERR_SLICE_INVALID_SUBSEQUENCE' ) {
				throw new Error( format( '1j7En', property ) );
			}
			if ( s.code === 'ERR_SLICE_TOO_MANY_DIMENSIONS' ) {
				throw new RangeError( format( '1j7Ev', target.shape.join( ',' ), replace( prop, /\.\.\.,/, '' ).split( ',' ).length ) );
			}
			...
			if ( s.code === 'ERR_SLICE_OUT_OF_BOUNDS' ) {
				if ( strict ) {
					throw new RangeError( format( '1j7Ep', shape.join( ',' ) ) );
				}
	...
	// Case: empty string
		...
		throw new RangeError( format( '1j7Ev', target.shape.join( ',' ), 0 ) );
	}
	return s;
}
